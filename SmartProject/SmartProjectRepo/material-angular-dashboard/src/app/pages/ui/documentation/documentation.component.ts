...
export class DocumentationComponent implements OnInit {
  ...
  projects = [];
  projectsTable = [];
  renderDoc: boolean = false;
  passData: PassToReleases;
  public numPage;
  public projectHeaders = this.projectService.getProjectsDocHeaders();
  userId: any;
  ...
  renderDocu(projectId, index) {
    if(this.renderDoc == false) {
      ...
      this.passData.projectId = projectId;
      this.passData.projectName = this.projects[index][1];
      setTimeout(() => this.renderDoc = true, 500);
    }
  }
